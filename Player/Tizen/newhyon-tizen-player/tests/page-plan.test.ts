...
describe('buildPagePlan', () => {
  ...
  it('페이지 볼륨을 0~100 범위로 정규화한다', () => {
    ...
    expect(buildPagePlan({ PIC_Elements: [] }, 'playlist').hasExplicitVolume).toBe(false);
    expect(buildPagePlan({ PIC_Volume: 42, PIC_Elements: [] }, 'playlist').hasExplicitVolume).toBe(true);
  });
